...
crud_cliente.leer = (req,res)=>{
    ...
    conectar.query('SELECT clientes.id_clientes,clientes.nit,clientes.nombres,clientes.apellidos,clientes.direccion,clientes.telefono, DATE_FORMAT(clientes.fecha_nacimiento,"%d-%m-%Y") as fecha_nacimiento FROM clientes;',(error,results)=>{
        if (error){
            throw error;
...
crud_cliente.cud = (req,res)=>{
    ...
    if (btn_actualizar){
        conectar.query('update clientes SET ? where id_clientes = ?',[{nit:nit,nombres:nombres, apellidos:apellidos,direccion:direccion,telefono:telefono,fecha_nacimiento:fecha_nacimiento},id], (error, results)=>{
            if(error){
                console.log(error);
            }else{   
                res.redirect('/');         
